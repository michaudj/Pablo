...
limit_value_for_bad_type=-1 #very imortant parameter that governs diversity. if two stimuli only have bad compatible types, nwe primitives can emerge. thus, if this value is high - high diversity and if low, low diversity.
...
#original emergence of types               
            if u == positive_reinforcement and not ts1:                
...
#                        if len(assigned_types[0])==2 and len(assigned_types[1]) ==2:
...
#typings will be made
        ...
        print("type 1 and 2 come from choose compatible pair")
...
#om assigned types är mer än en lång (egentligen mer än noll) så är vi inte postborder och vi kan ha a/b,b elkler a\b,b innan. i första fallet måste a ändras till primitive. i andra fallet måste b ändras till primitive
            ...
            assigned_types,sentence_condition,ts1=assign_two_types_that_reduce_to_x(type1[0][0],assigned_types,ts1) #the previous types should reduce to the first element in type 1#Nmakes sure the two previouslu assigned types arechanged to reduce to primitive
            if len(type1[0])>1: 
...
#om type1 och type2 är chunk-kompatibla så ska en inherited type_1 som ärvs till nästa runda skapas. den kan bara skapas här eftersom compatible_type kan ha valts probabilistiskt bland flera möjliga och därför "vet" vi inte förrän här vilak typ1 och typ2 är och hur de är kompatibla.    
    ...
    if ts1 and ts2:
        ...
        if type1[0]==[0]:
            b_values[0]+=type1[1]                                                  #add type1 value to behaviour border
#        else:
#          for i in range(len(b_values[1:])): #SKIP, if not both are types and compatible, types should not support anything
